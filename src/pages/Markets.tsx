...
export function Markets({ onNavigate }: { onNavigate: (tab: string, symbol?: string) => void }) {
  ...
  const filteredAndSortedPairs = useMemo(() => {
    ...
    if (sortType === 'gainers') {
      withTickers.sort((a, b) => parseFloat(b.ticker?.priceChangePercent || '-999') - parseFloat(a.ticker?.priceChangePercent || '-999'));
    } else if (sortType === 'losers') {
      withTickers.sort((a, b) => parseFloat(a.ticker?.priceChangePercent || '999') - parseFloat(b.ticker?.priceChangePercent || '999'));
    } else if (sortType === 'volume') {
      withTickers.sort((a, b) => {
        const vA = parseFloat(a.ticker?.quoteVolume24h || '-1');
        const vB = parseFloat(b.ticker?.quoteVolume24h || '-1');
        return vB - vA;
      });
    } else if (sortType === 'price') {
      withTickers.sort((a, b) => parseFloat(b.ticker?.lastPrice || '-1') - parseFloat(a.ticker?.lastPrice || '-1'));
    }
      ...
      <div className="flex-1 overflow-auto px-4 mt-2">
        ...
        <div className="flex flex-col space-y-1">
          ...
          {filteredAndSortedPairs.map(({ pair, ticker }) => {
            ...
            const chg = ticker ? parseFloat(ticker.priceChangePercent) : NaN;
              ...
              >
                ...
                <div className="flex-1 sm:hidden">
                  ...
                  <div className="text-[10px] text-gray-500 mt-0.5">
                    Vol {ticker?.quoteVolume24h ? (parseFloat(ticker.quoteVolume24h) / 1000000).toFixed(2) + 'M' : '--'}
                  </div>
                </div>
                ...
                <div className="w-24 text-right flex flex-col justify-center sm:hidden">
                  <div className={`font-bold text-sm ${isPositive ? 'text-emerald-500' : 'text-red-500'}`}>
                    {ticker?.lastPrice ? parseFloat(ticker.lastPrice).toLocaleString(undefined, { minimumFractionDigits: pair.quantityPrecision }) : '--'}
                  </div>
                </div>
                ...
                <div className="hidden sm:flex w-24 justify-end font-medium text-gray-200">
                    {ticker?.lastPrice ? parseFloat(ticker.lastPrice).toLocaleString() : '--'}
                </div>
                ...
                <div className="hidden sm:flex w-24 justify-end text-sm text-gray-400">
                    {ticker?.high24h ? parseFloat(ticker.high24h).toLocaleString() : '--'}
                </div>

                <div className="hidden sm:flex w-24 justify-end text-sm text-gray-400">
                    {ticker?.low24h ? parseFloat(ticker.low24h).toLocaleString() : '--'}
                </div>

                <div className="hidden sm:flex w-28 justify-end text-sm text-gray-400">
                    {ticker?.quoteVolume24h ? parseFloat(ticker.quoteVolume24h).toLocaleString(undefined, { maximumFractionDigits: 0 }) : '--'}
                </div>
              </div>
            );
          })}
