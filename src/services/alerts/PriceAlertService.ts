  ...
  private load() {
    try {
      if (typeof localStorage !== 'undefined') {
        ...
        if (stored) {
          const parsed = safeParseArray<PriceAlert>(stored, item => (
            item && typeof item.id === 'string' && typeof item.symbol === 'string' && isValidFinancialString(item.targetPrice)
          ));
          if (parsed.length > 0 || stored.trim() === '[]') {
            this.alerts = parsed;
          }
        }
      }
